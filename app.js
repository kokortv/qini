...
function inventoryFromData(x){
  ...
  deliveries.forEach(d=>(d.items||[]).forEach(i=>{
    ...
    const qty=+i.qty||0;
    if(!qty)return;
    batches.push({name:i.name,pack:i.pack||'',packWeight:i.packWeight||'',stock:qty,cost:+i.cost||0,markup:Math.round(+i.markup||0),retailTotal:+i.retailTotal||0,warehouse:d.warehouse||'',deliveryId:d.id||'',deliveryDate:d.date||'',supplier:d.supplier||'',account:d.account||''});
  }));
...
function renderMenuPage(menu){
  ...
  document.querySelector('#app').innerHTML=`<style>${styles}</style><div class="menu-page"><header class="menu-header"><div class="menu-brand">Q</div><h1 class="menu-title">${esc(menu.title||'Меню')}</h1><div class="menu-sub">Актуально на ${new Date().toLocaleDateString('ru-RU')}</div></header>${items.length?`<div class="menu-list">${items.map(i=>`<div class="menu-item"><div><div class="menu-item-name">${esc(i.n||'')}${i.pw?' <span style="color:#8c93a5;font-weight:400">'+esc(i.pw)+'</span>':''}</div>${i.p?`<div class="menu-item-pack">${esc(i.p)}</div>`:''}</div><div class="menu-item-price">${money(i.pr)}</div></div>`).join('')}</div>`:'<div class="menu-empty">Меню пустое</div>'}<footer class="menu-footer">Сформировано в qini</footer></div>`;
}
...
function openCreateMenu(){
  ...
  el('#menuForm').onsubmit=async e=>{
    ...
    const items=picks.map(i=>{const p=products[i];return {n:p.name,p:p.pack,pw:p.packWeight||'',pr:Math.round(p.cost*(1+p.markup/100))}});
